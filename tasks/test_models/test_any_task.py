"""
New(0501): evenly evaluate each task for the same number of --eval_each_task times. 

Use one script to evaluate any task.
Different date may correspond to different image sizes in the dataset.. make sure to render with correct size!
-   Basketball_hard:
    0418: 100x180; 0419: 150x270
-   PickPlace:
    0418: 150x270; 0419: 200x360!
"""
...
def build_tvf_formatter(config, env_name='stack'):
    ...
    def resize_crop(img):
        ...
        obs = resized_crop(obs, top=top, left=left, height=box_h, width=box_w,
                        size=(height, width))
        # if env_name == 'place':
        #     obs = RandomResizedCrop(size=(90, 130), scale=[1, 1], ratio=[1.44,1.44])(obs)
        # # centercrops = transforms.Compose([
        #     transforms.CenterCrop(size=(int(height*0.8), int(width*0.8))),
        #     transforms.Resize((height, width))])
        # centercrops = transforms.RandomResizedCrop(size=(height, width), scale=(0.8, 0.8))
        # obs = centercrops(obs)
...
def build_env_context(img_formatter, T_context=4, ctr=0, env_name='nut', 
    ...
    if isinstance(context[0], np.ndarray):
        ...
        # _B, T, _, _H, _W = context.shape 
        # fig, axs = plt.subplots(nrows=1, ncols=T, figsize=(T*5, 4), subplot_kw={'xticks': [], 'yticks': []})
        # fig.subplots_adjust(left=0.03, right=0.97, hspace=0.3, wspace=0.05)
        # for img_index in range(T):
        #     show_con = context[0][img_index].cpu().numpy() * STD + MEAN 
        #     axs[img_index].imshow(show_con.transpose(1,2,0))
        # plt.tight_layout()
